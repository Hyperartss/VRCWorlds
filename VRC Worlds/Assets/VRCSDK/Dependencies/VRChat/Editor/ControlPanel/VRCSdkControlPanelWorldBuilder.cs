...
namespace VRC.SDKBase.Editor
{
    public class VRCSdkControlPanelWorldBuilder : IVRCSdkControlPanelBuilder
    {
        ...
        public virtual void ShowBuilder()
        {
            ...
            else if (_scenes.Length == 1)
            {
                ...
                try
                {
                    ...
                    if (!setupRequired)
                    {
                        if (!_builder.CheckedForIssues || !Bloodborne.VRChat.CheckedForIssues)
                        {
                            _builder.ResetIssues();
                            OnGUISceneCheck(_scenes[0]);
                            _builder.CheckedForIssues = true;
                            Bloodborne.VRChat.CheckedForIssues = true;
                        }
        ...
        if (isSdk3Scene == false)
        {
            ...
            if (!string.IsNullOrEmpty(vrcFilePath) && ValidationHelpers.CheckIfAssetBundleFileTooLarge(ContentType.World, vrcFilePath, out int fileSize, isMobilePlatform))
            {
            ...
            _builder.OnGUIWarning(
                scene,
                Bloodborne.VRChat.Warning.World(),

...
public partial class VRCSdkControlPanel : EditorWindow
{
    ...
    static bool OnAccountGUI()
    {
        ...
        if (signingIn)
        {
            ...
            EditorGUILayout.LabelField(username, titleStyle);
        }
        else if (APIUser.IsLoggedIn)
        {
            ...
            Bloodborne.VRChat.Auth.In(Logout);
        }
        else
        {
            InitAccount();

            if (serverEnvironment != Bloodborne.VRChat.Auth.Server)
            {
                serverEnvironment = Bloodborne.VRChat.Auth.Server;
            }

            if (username != Bloodborne.VRChat.Auth.Username)
            {
                username = Bloodborne.VRChat.Auth.Username;
            }

            if (password != Bloodborne.VRChat.Auth.Password)
            {
                password = Bloodborne.VRChat.Auth.Password;
            }
            ...
            Bloodborne.VRChat.Auth.Out(SignIn, Logout, Code, serverEnvironment);
        }
    ...
    private static void AttemptLogin()
    {
        APIUser.Login(username, password,
            delegate (ApiModelContainer<APIUser> c)
            {
                ...
                Bloodborne.VRChat.Auth.Storage(user, Logout);
            },
            delegate (ApiModelContainer<APIUser> c)
            {
                Logout();
    ...
    private void AccountDestroy()
    {
        signingIn = false;
        isInitialized = false;
        Bloodborne.VRChat.Auth.Reload = true;
    }
}
